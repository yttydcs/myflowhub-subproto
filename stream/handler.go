...
func (h *Handler) routeCoordinatorRequest(ctx context.Context, conn core.IConnection, hdr core.IHeader, payload []byte, requester uint32, perm string, producer, consumer uint32, sendErr func(int, string), handleLocal func()) {
	...
	local := srv.NodeID()
	cm := srv.ConnManager()
	producerLocal, producerConn, okProducer := resolveRoute(cm, local, producer)
	consumerLocal, consumerConn, okConsumer := resolveRoute(cm, local, consumer)
	...
	if !producerLocal && !consumerLocal && producerConn != nil && consumerConn != nil && producerConn.ID() == consumerConn.ID() {
		nextNode := connNodeID(producerConn)
		if nextNode == 0 {
			sendErr(500, "invalid route")
			return
		}
		...
		fwdHdr.WithTargetID(nextNode)
		h.sendToConn(ctx, producerConn, fwdHdr, payload)
		return
	}
